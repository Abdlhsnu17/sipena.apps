...
if [ -z "$whatsapp_url" ] && [ -z "$sms_url" ] && { [ -z "$smtp_user" ] || [ -z "$smtp_pass" ]; }; then
...
docker inspect sipena-apps-backend-1 --format '{{range .Config.Env}}{{println .}}{{end}}' |
  awk -F= '/^(WHATSAPP_OTP_WEBHOOK_URL|WHATSAPP_OTP_WEBHOOK_TOKEN|SMS_OTP_WEBHOOK_URL|SMS_OTP_WEBHOOK_TOKEN|SMTP_HOST|SMTP_USER|SMTP_PASS|SMTP_FROM|EMAIL_FROM)=/ {
