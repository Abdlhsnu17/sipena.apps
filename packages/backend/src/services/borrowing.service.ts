...
export class BorrowingService {
  ...
  canActorViewBorrowing(
    ...
    if (hasAnyRole(actorRole, ['staff_pj', 'staff pj'])) {
      const normalizedActorWorkUnit = normalizeComparableText(actorWorkUnit);
      const normalizedBorrowerWorkUnit = normalizeComparableText(this.getBorrowerWorkUnit(borrowing));
      if (normalizedActorWorkUnit && normalizedActorWorkUnit === normalizedBorrowerWorkUnit) {
        return true;
      }
    }

    const borrowingUserId = Number(borrowing.userId ?? borrowing.user_id);
    return actorUserId !== null && Number.isFinite(borrowingUserId) && borrowingUserId === actorUserId;
  }
