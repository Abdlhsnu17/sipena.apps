...
function ClassCard({ name, color, badge, properties, methods }: {
  ...
  return (
    <div className={`rounded-xl border bg-white dark:bg-slate-900 overflow-hidden shadow-lg ${colorClasses[color]?.split(' ').slice(1).join(' ')}`}> {/* ...existing code... */} </div>
  )
}
...
function ActivityFlow({ title, color, steps }: {
  ...
  return (
    <div className="rounded-xl p-5 border"> {/* ...existing code... */} </div>
  )
}
...
function TableCard({ name, color, columns }: {
  ...
  return (
    <div className="rounded-xl border bg-white dark:bg-slate-900 overflow-hidden shadow-lg"> {/* ...existing code... */} </div>
  )
}
