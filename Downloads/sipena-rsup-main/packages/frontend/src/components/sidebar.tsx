...
export default function Sidebar({ isCollapsed, toggleSidebar }: SidebarProps) {
  ...
  const updateMobileScrollHint = useCallback(() => {
    ...
    const canScrollMore = panel.scrollHeight - panel.clientHeight > 12
    const isNearBottom = panel.scrollTop + panel.clientHeight >= panel.scrollHeight - 12

    setShowMobileScrollHint(canScrollMore && !isNearBottom)
  }, [])
  ...
  useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(updateMobileScrollHint)
  ...
  const SidebarContent = () => (
    <>
      <div className="p-4 border-b border-border" data-sidebar-header>
        {isCollapsed ? (
      ...
      {currentUser && !isCollapsed && (
        <div className="px-4 pb-0 z-0" data-sidebar-secondary>
      ...
      {currentUser && !isCollapsed && (
        <div className="px-4 py-2 z-0" data-sidebar-secondary>
      ...
      <div className="p-4 border-t border-border" data-sidebar-secondary>
        <div className={cn(
      ...
      >
        ...
        >
          ...
          <div
            aria-hidden="true"
            className={cn(
              "pointer-events-none absolute inset-x-2 bottom-2 z-30 transition-opacity duration-200",
              showMobileScrollHint ? "opacity-100" : "opacity-0"
            )}
          >
            <div className="rounded-full border border-border/70 bg-background/90 px-2 py-1 text-center text-[10px] font-semibold tracking-[0.08em] text-muted-foreground shadow-sm backdrop-blur-sm">
              Scroll untuk lihat menu lainnya
            </div>
          </div>
        </aside>
      </div>
