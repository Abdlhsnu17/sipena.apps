...
export default function UnggahanPage() {
  ...
  const clearUploadSelection = (event?: MouseEvent<HTMLButtonElement>) => {
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (uploadResults.length > 0) {
        ...
        setUploadedReportName(files.length > 1 ? `${files.length} file dipilih` : files[0].name)
      }
    } catch (error) {
  ...
  const handleDeleteUpload = async (id: number) => {
    ...
    if (!window.confirm("Hapus unggahan ini?")) return
    try {
      setDeletingId(id)
  ...
  return (
    <div className="flex-1 bg-background">
      <main className="flex min-h-screen flex-col overflow-y-auto bg-gradient-to-br from-slate-50 via-white to-teal-50/40 dark:from-slate-950 dark:via-slate-900 dark:to-teal-950/30">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 md:gap-8 md:px-6 lg:px-8">
        <section className="rounded-[32px] bg-gradient-to-br from-slate-100 via-white to-cyan-50/80 dark:from-slate-950 dark:via-slate-900 dark:to-teal-950/30 shadow-[0_30px_60px_rgba(14,165,233,0.12)]">
          <div className="p-6 md:p-8 space-y-6">
            ...
            <Card className="rounded-[28px] border border-slate-200/60 bg-white/90 shadow-[0_30px_60px_rgba(14,165,233,0.15)] dark:border-slate-800/70 dark:bg-slate-900/70">
              <CardContent className="space-y-6">
                <div className="flex items-center gap-3 rounded-2xl border border-teal-100/80 bg-gradient-to-br from-teal-50/80 via-emerald-50 to-cyan-50 p-4 dark:border-slate-800/60 dark:bg-slate-900/50">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-white text-teal-600 shadow-inner dark:text-teal-400">
                ...
                <div className="rounded-2xl border border-dashed border-teal-200/70 bg-white/90 p-5 dark:bg-slate-950/60">
                  <div className="flex flex-col gap-3">
                    <label
                      ...
                      className="flex items-center justify-between gap-3 rounded-2xl border border-teal-100/80 bg-gradient-to-r from-teal-50 to-cyan-50 px-4 py-3 text-sm font-semibold text-teal-700 hover:cursor-pointer hover:border-teal-200 dark:bg-slate-900/40 dark:text-teal-200"
                    >
                      <span className="truncate">
                        {uploadedReportName ? `Terpilih: ${uploadedReportName}` : "Pilih file laporan..."}
                      </span>
                      <span className="text-xs font-semibold uppercase tracking-[0.4em] text-teal-700">Unggah</span>
                    </label>
                    {uploadedReportName && (
                    ...
                    <input id="report-upload" type="file" className="sr-only" onChange={handleFileUpload} multiple />
                    <p className="text-xs text-muted-foreground">
                  ...
                  if (textPreviewContent) {
                    return (
                      <div className="rounded-lg border bg-muted/30 p-4">
                        <pre className="max-h-[480px] overflow-auto whitespace-pre-wrap text-sm text-muted-foreground">
                          {textPreviewContent}
                        </pre>
                      </div>
                    )
