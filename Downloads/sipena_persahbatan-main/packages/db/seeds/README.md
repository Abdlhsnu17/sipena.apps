...
2. Open `http://localhost:8081`, log in as `root` and import `packages/database/schema.sql` into the `sipena_db_local` database.
3. Optionally import `packages/database/seeds/schema.sql` or `schema.localhost.sql` whenever you need a documented copy of the schema for phpMyAdmin/backup purposes.
