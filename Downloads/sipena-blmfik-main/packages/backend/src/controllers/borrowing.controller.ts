    ...
    try {
      ...
      if (actorId && result.success && result.data) {
        await recordUserActivity({
          ...
          action: 'create',
          description: `Membuat peminjaman alat ${result.data.borrowingCode || `#${result.data.id}`}`,
          metadata: {
    ...
    try {
      ...
      if (actorId && result.data) {
        await recordUserActivity({
          ...
          feature: 'peminjaman_alat',
          action: 'update',
          description: `Mengubah data peminjaman ${result.data.borrowingCode || `#${result.data.id}`}`,
          metadata: {
            borrowingId: result.data.id,
            status: result.data.status,
          },
    ...
    try {
      ...
      if (actorId && result.data) {
        await recordUserActivity({
          ...
          feature: 'peminjaman_alat',
          action: 'approve',
          description: `Menyetujui peminjaman ${result.data.borrowingCode || `#${result.data.id}`}`,
          metadata: { borrowingId: result.data.id, status: result.data.status },
        });
      }
    ...
    try {
      ...
      if (actorId && result.data) {
        await recordUserActivity({
          ...
          feature: 'peminjaman_alat',
          action: 'reject',
          description: `Menolak peminjaman ${result.data.borrowingCode || `#${result.data.id}`}`,
          metadata: {
            borrowingId: result.data.id,
            status: result.data.status,
            reason,
    ...
    try {
      ...
      if (actorId && result.data) {
        await recordUserActivity({
          ...
          action: 'return',
          description: `Mengembalikan alat untuk peminjaman ${result.data.borrowingCode || `#${result.data.id}`}`,
          metadata: {
            borrowingId: result.data.id,
            condition,
            status: result.data.status,
    ...
    try {
      ...
      if (actorId && result.data) {
        await recordUserActivity({
          ...
          feature: 'pengembalian_alat',
          action: 'validate',
          description: `Memvalidasi pengembalian alat ${result.data.borrowingCode || `#${result.data.id}`}`,
          metadata: {
            borrowingId: result.data.id,
            status: result.data.status,
          },
    ...
    try {
      ...
      if (actorId) {
        await recordUserActivity({
          ...
          feature: 'peminjaman_alat',
          action: 'delete',
          description: `Menghapus data peminjaman ${beforeDelete.data?.borrowingCode || `#${id}`}`,
          metadata: {
