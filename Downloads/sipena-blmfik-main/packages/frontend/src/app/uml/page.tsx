      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          <div className="flex flex-wrap items-center gap-4">
            <TabsList className="grid grid-cols-2 lg:grid-cols-5 gap-2 h-auto p-2 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 rounded-xl flex-1 min-w-70">
              {visibleTabs.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
          ...
          <TabsContent value="usecase" className="space-y-6">
            <Card className="border-0 shadow-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
              ...
              <CardContent className="p-6">
                ...
                {/* System Overview */}
                <div className="mt-8 p-6 bg-linear-to-r from-gray-50 to-slate-50 dark:from-slate-800/50 dark:to-slate-900/50 rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {useCaseSummaryCards.map((card, i) => (
                      <div key={i} className={`text-center p-4 rounded-xl ${card.containerClass}`}>
                        <p className={`font-semibold mb-3 ${card.titleClass}`}>{card.title}</p>
...
function RoleUseCaseCard({ card }: { card: UseCaseRoleCard }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border p-6 ${card.containerClass}`}>
      <div className={`absolute top-0 right-0 h-32 w-32 rounded-full -mr-16 -mt-16 blur-2xl ${card.glowClass}`}></div>
      <div className="relative">
        <div className="mb-4 flex items-center gap-3">
          <div className={`rounded-xl p-3 shadow-lg ${card.iconWrapClass}`}>
            <span className="text-xl">{card.roleIcon}</span>
          </div>
          <div>
            <h3 className={`text-lg font-bold ${card.titleClass}`}>{card.title}</h3>
        ...
        <div className="space-y-3">
          {card.items.map((item, i) => (
            <div
              key={`${card.key}-${i}`}
              className={`flex cursor-default items-start gap-3 rounded-xl p-3 transition-all ${
                item.disabled
                  ? "border border-red-200/50 bg-red-50/60 dark:border-red-800/50 dark:bg-red-950/20"
            ...
            >
              ...
              <div className="flex-1">
                <p
                  className={`text-sm font-medium ${
                    item.disabled ? "text-red-600 dark:text-red-400" : "text-foreground"
                  }`}
                >
                  {item.title}
                </p>
              ...
              {!item.disabled && <ChevronRight className={`h-4 w-4 ${card.chevronClass}`} />}
            </div>
          ))}
        </div>
      </div>
