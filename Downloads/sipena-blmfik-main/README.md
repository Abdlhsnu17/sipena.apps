...
2. Visit `http://localhost:8081`, log in with `root` / `root`, create or select the `sipena_db_local` database, and import `packages/db/schema.sql`.
...
> **Tip:** the frontend will use the value of `NEXT_PUBLIC_API_URL` when making
> API requests. For local development this is set to `http://localhost:4000`,
> but in a real deployment you can either point it at your server or leave it
> blank/undefined so that requests stay relative (e.g. `/api/maintenance`).
> Leaving it blank also prevents `localhost` from appearing in error messages
> or network logs when schedules are created.
...
- `packages/db`: schema, migrations, and seeds
...
- Otentikasi dan manajemen pengguna berbasis peran (admin, leader, staff, staff_pj, teknisi, user/pengguna)
...
- Basis Data: SQL (lihat folder `packages/db`)
...
- **Admin:** Akses penuh ke seluruh fitur (inventaris, pemeliharaan, peminjaman, laporan, manajemen pengguna)
- **Leader:** Akses operasional luas untuk inventaris, peminjaman, pengembalian, pemeliharaan, laporan, dan manajemen pengguna penuh
- **Staff:** Akses ke pemeliharaan, jadwal pemeliharaan, peminjaman alat, pengembalian alat, pengaturan, dan laporan
- **Pengguna:** Akses self-service terbatas ke dashboard, dokumentasi UML, peminjaman alat, pengembalian alat, dan pengaturan akun
- **Staff PJ:** Akses ke input data aset medis, input data aset non-medis, pemeliharaan, jadwal pemeliharaan, peminjaman alat, pengembalian alat, pengaturan, dan laporan
- **Teknisi:** Fokus pada eksekusi pemeliharaan (update status, penyelesaian, pembatalan, catatan teknis)
