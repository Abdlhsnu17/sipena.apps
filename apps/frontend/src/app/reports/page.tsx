...
const chartColors = {
  ...
  borrowing: "var(--chart-5)",
  usage: "var(--chart-1)",
  neutral: "var(--muted-foreground)",
}
const CHART_GRID_COLOR = "var(--chart-grid)"
const CHART_ACCENT_COLOR = "var(--chart-4)"
const CHART_BLUE_COLOR = "var(--chart-2)"
...
function MetricCard({
  ...
  return (
    <Card className="rounded-lg border-slate-200 py-0 shadow-sm dark:border-slate-800/35">
...
function ChartCard({
  ...
  return (
    <Card className={`rounded-lg border-slate-200 py-0 shadow-sm dark:border-slate-800/35 ${className}`}>
      <CardHeader className="border-b border-slate-100 px-4 py-3 dark:border-slate-800/35">
...
export default function ReportsPage() {
  ...
  const canSelectReportUser = true
  ...
  return (
    <div className="min-w-0">
      <div className="w-full space-y-5">
        ...
        <Card className="rounded-lg border-slate-200 py-0 shadow-sm dark:border-slate-800/35">
          <CardHeader className="border-b border-slate-100 px-4 py-3 dark:border-slate-800/35">
            <CardTitle className="text-sm font-semibold">Ekspor Laporan</CardTitle>
          ...
          <CardContent className="grid gap-5 px-4 py-4 xl:grid-cols-[minmax(0,1fr)_18rem]">
            <div className="space-y-4">
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-20 rounded-md border px-3 py-2 text-left transition ${
                        active
                          ? "border-slate-900 bg-slate-900 text-white shadow-sm dark:border-teal-500/60 dark:bg-teal-500/20 dark:text-teal-100"
                          : "border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50 dark:border-slate-800/35 dark:bg-transparent dark:text-slate-300 dark:hover:border-slate-700 dark:hover:bg-slate-800/40"
                      }`}
                    >
                      <span className="block text-sm font-semibold">{option.label}</span>
                      <span className={`mt-1 block text-xs ${active ? "text-slate-200 dark:text-teal-200/80" : "text-slate-500 dark:text-slate-400"}`}>{option.description}</span>
                    </button>
                  )
                })}
              </div>
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] lg:items-end">
                <div className="space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-normal text-slate-500 dark:text-slate-400">Periode</p>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <label className="space-y-1 text-xs font-medium text-slate-600 dark:text-slate-300">
                    ...
                    <input
                      ...
                      className={`h-10 w-full rounded-md border bg-white px-3 text-sm text-slate-700 shadow-xs outline-none transition focus:border-slate-400 dark:bg-slate-900/40 dark:text-slate-200 ${
                        dateRangeInvalid ? "border-red-300 dark:border-red-500/60" : "border-slate-200 dark:border-slate-800/35"
                      }`}
                    />
                  </label>
                  <label className="space-y-1 text-xs font-medium text-slate-600 dark:text-slate-300">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  {showUserFilter ? (
                    canSelectReportUser ? (
                      ...
                      <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-800/35 dark:bg-slate-900/40">
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">User aktivitas</p>
                        <p className="mt-1 text-sm font-semibold text-slate-800 dark:text-slate-100">{selectedUserLabel}</p>
                      </div>
                    )
                  ) : null}
                </div>
            ...
            <div className="border-t border-slate-200 pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0 dark:border-slate-800/35">
              <p className="text-xs font-semibold uppercase tracking-normal text-slate-500 dark:text-slate-400">Unduh file</p>
              <p className="mt-1 text-sm font-semibold text-slate-950 dark:text-slate-50">{selectedReportType.label}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {activeExportFilterLabels.map((label) => (
                  <span key={String(label)} className="rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-600 dark:border-slate-800/35 dark:bg-slate-900/40 dark:text-slate-300">
                    {label}
                  </span>
                ))}
              </div>
              <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
                ...
                <Button type="button" variant="outline" className="justify-start bg-white dark:bg-transparent" onClick={() => void handleExport("csv")} disabled={exporting !== null || dateRangeInvalid}>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
          <MetricCard title="Total Ruangan" value={formatNumber(totalRooms)} description="Ruangan medis dan non medis" icon={Boxes} accentClassName="bg-sky-50 text-sky-700 dark:bg-sky-400/10 dark:text-sky-300" />
          <MetricCard title="Ruangan Medis" value={formatNumber(totalMedicalRooms)} description="Ruangan inventaris klinis" icon={Stethoscope} accentClassName="bg-cyan-50 text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-300" />
          <MetricCard title="Ruangan Non Medis" value={formatNumber(totalNonMedicalRooms)} description="Ruangan sarana pendukung" icon={PackageCheck} accentClassName="bg-indigo-50 text-indigo-700 dark:bg-indigo-400/10 dark:text-indigo-300" />
          <MetricCard title="Inventaris Terinput" value={formatNumber(totalInventoryDetails)} description="Detail aset di semua ruangan" icon={Boxes} accentClassName="bg-violet-50 text-violet-700 dark:bg-violet-400/10 dark:text-violet-300" />
          <MetricCard title="Inventaris Medis" value={formatNumber(totalMedicalInventoryDetails)} description="Detail aset klinis terinput" icon={Stethoscope} accentClassName="bg-teal-50 text-teal-700 dark:bg-teal-400/10 dark:text-teal-300" />
          <MetricCard title="Inventaris Non Medis" value={formatNumber(totalNonMedicalInventoryDetails)} description="Detail sarana terinput" icon={PackageCheck} accentClassName="bg-blue-50 text-blue-700 dark:bg-blue-400/10 dark:text-blue-300" />
          <MetricCard title="Pemeliharaan" value={formatNumber(maintenance.length)} description="Semua jadwal tercatat" icon={Hammer} accentClassName="bg-amber-50 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300" />
          <MetricCard title="Peminjaman" value={formatNumber(borrowings.length)} description="Sesi pinjam pakai" icon={CalendarCheck2} accentClassName="bg-rose-50 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300" />
          <MetricCard title="Penggunaan" value={formatNumber(totalUsageCount)} description="Akumulasi pemakaian" icon={TrendingUp} accentClassName="bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300" />
        </div>

        <Tabs defaultValue="overview" className="gap-4">
          <div className="overflow-x-auto">
            <TabsList className="h-10 min-w-max bg-white p-1 shadow-sm dark:bg-slate-900/60">
              ...
              <ChartCard title="Komposisi Data Inventaris" description="Ruangan dibanding detail inventaris terinput">
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={inventorySummaryData} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                    ...
                    <Bar dataKey="inventaris" fill={CHART_ACCENT_COLOR} name="Inventaris Terinput" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </ChartCard>
            </div>
              ...
              <ChartCard title="Pemeliharaan Per Bulan" description="Perbandingan jadwal tervalidasi dan belum validasi" className="xl:col-span-2">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={monthlyData} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                    ...
                    <Tooltip />
                    <Legend wrapperStyle={{ fontSize: 11 }} />
                    <Bar dataKey="completed" stackId="maintenance" fill={CHART_BLUE_COLOR} name="Tervalidasi" radius={[6, 6, 0, 0]} />
              ...
              <ChartCard title="Ringkasan Peminjaman" description="Jumlah sesi berdasarkan status utama">
                <div className="grid min-h-75 content-center gap-3 px-2">
                  {borrowingStatusData.map((item) => (
                    <div key={item.status} className="flex items-center justify-between rounded-md border border-slate-200 px-3 py-2 dark:border-slate-800/35">
                      <span className="text-sm text-slate-600 dark:text-slate-300">{item.status}</span>
                      <span className="text-sm font-semibold text-slate-950 dark:text-slate-50">{formatNumber(item.total)}</span>
                    </div>
                  ))}
                </div>
              </ChartCard>
